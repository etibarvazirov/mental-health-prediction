...
class FusionModel(nn.Module):
    def __init__(self, input_dim=780):
        super().__init__()
...
sleep_duration = preset["sleep"] if preset else st.sidebar.slider("Yuxu müddəti (saat)", 0.0, 12.0, 7.0)
...
if st.sidebar.button("🔮 Proqnoz Et"):
    ...
    fusion_input = np.concatenate([bert_emb, numeric_scaled])
    ...
    # pred is in range 0–1 → convert to 1–10 scale
    ...
    if st.checkbox("📊 Qrafikləri göstər"):
        ...
        with col1:
            st.image("images/fig4_shap_clean.png", caption="SHAP — Faktorların təsiri")
        with col2:
            st.image("images/fig1_prediction_vs_actual.png", caption="Prediction vs Actual")

        col3, col4 = st.columns(2)
        with col3:
            st.image("images/fig3_pca.png", caption="BERT PCA analizi")
        with col4:
            st.image("images/fig2_model_comparison.png", caption="Model müqayisəsi")

        st.image("images/fusion_architecture.png", caption="Fusion arxitektura")

else:
    st.info("Məlumatları doldurun və 'Proqnoz Et' düyməsinə basın.")
